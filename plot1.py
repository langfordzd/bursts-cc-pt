...
def amp_dur(com1,com2,com3):
    ...
    co = com1['freq'][com1['which']=='pt_com']
    ...
    pt = com2['freq'][com2['which']=='pt']
    co = com2['freq'][com2['which']=='pt_com']
    ...
    pt = com3['freq'][com3['which']=='pt']
    co = com3['freq'][com3['which']=='pt_com']
...
#%%
def print_stats(com,measure,dataset):
    ...
    b = np.array(means.loc[(slice(None),slice(None), 'pt')])
    print('CC:', '{:.3}'.format(np.mean(a)), '(', '{:.3}'.format(np.std(a)), ')', 
          'PT','{:.3}'.format(np.mean(b)), '(', '{:.3}'.format(np.std(b)), ')')
    toShuff = a-b
    ...
    if measure == 'freq':
        ...
        #%%
def print_stats_old(com,measure,dataset):
    from scipy.stats import wilcoxon
    from scipy.stats import ttest_rel
    import numpy as np

    means = com.groupby(['loc','chan','which'])[measure].mean()
    ptm = means.loc[(slice(None),slice(None), 'pt')].mean()
    ccm = means.loc[(slice(None),slice(None), 'cc')].mean()
    pts = means.loc[(slice(None),slice(None), 'pt')].std()
    ccs = means.loc[(slice(None),slice(None), 'cc')].std()
    print('--------------------------------------------')
    print(dataset)
    print(measure)
    print([' PT:', '{:.3}'.format(ptm), '{:.3}'.format(pts),
           'vs CC:', '{:.3}'.format(ccm), '{:.3}'.format(ccs)])
    print(wilcoxon(means.loc[(slice(None),slice(None), 'cc')]-means.loc[(slice(None),slice(None), 'pt')]))
    print(ttest_rel(means.loc[(slice(None),slice(None), 'cc')],means.loc[(slice(None),slice(None), 'pt')]))
    #means.loc[(slice(None), 'pt'), 'duration'].mean()
...
#%%
#%%
def amp_dur_norm(com1,com2,com3):
    ...
    xtr.set_yticks([0, 0.25, 0.5, 0.75, 1])
    xtr.set_yticklabels(['0', '','','', '1'])
    xtr.set_xticks([0, 0.05])
    xtr.set_xticklabels(['0', '0.05'])
    xtr.tick_params(axis="both",direction="in")
    xtr.set(ylabel='',xlabel='')
    xtr.set_title('PTA')
    xtr.yaxis.set_label_coords(-0.05, 0.5)
