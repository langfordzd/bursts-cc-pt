...
def behav(l, bs, trials, which, ids):
    ...
    temp0['which'] = which
    # sns.countplot(data = temp0, x='count')
    # plt.show()
    bh['which'] = which
    
    return b, bh
#%%
def print_stats(com,dataset):
    from scipy.stats import wilcoxon
    from scipy.stats import ttest_rel
    print('---------------------------------')
    print(dataset)
    cc = com[(com['which']=='cc') | (com['which']=='pt_com')]
    print('CC')
    print(wilcoxon(cc['ratio_burst']-cc['ratio_behav']))
    print(ttest_rel(cc['ratio_burst'],cc['ratio_behav']))

    pt = com[(com['which']=='pt') | (com['which']=='pt_com')]
    print('PT')
    print(wilcoxon(pt['ratio_burst']-pt['ratio_behav']))   
    print(ttest_rel(pt['ratio_burst'],pt['ratio_behav']))
#%%    
def print_stats2(com, dataset, which, measure):
    from scipy.stats import ttest_rel
    ...
    import pandas as pd
    # p = pd.DataFrame(means).reset_index()
    # sns.violinplot(x='which', y=measure, hue="behav",
    #                 data=p, palette="Set2", split=True,inner="quartile")
    # plt.show()
    ...
    print(which)
    #print(wilcoxon(a-b))
    print('search:', np.mean(a), 'repeat', np.mean(b))
    print(ttest_rel(a,b))
...
#%%
def np_test(com,dataset,which):
    import numpy as np
    import pandas as pd
    pd.options.mode.chained_assignment = None 
    shuffles = 10000
    results = []
    cc = com[(com['which']==which) | (com['which']=='pt_com')]
    toShuff = cc['ratio_burst']-cc['ratio_behav']
    results.append(toShuff.mean())
    for i in range(1,shuffles):
        ones = np.random.choice([-1, 1], size=len(toShuff))
        t = toShuff*ones
    ...
    if which == 'cc':
         ...
         p = a/shuffles
         print([a,p])
         cc['over'] = cc['ratio_burst']>cc['ratio_behav']
         print([cc['over'].sum(), len(cc)])
    else:
         a = np.sum(results[1:]<results[0])
         p = a/shuffles
         print([a,p])
...
def plot_beh(pt_lfp_b,pt_lfp_c,cc_lfp_b,cc_lfp_c,pt_ecog_b,pt_ecog_c,cc_ecog_b,cc_ecog_c,ecog,marco,pablo,v,toSave):
    ...
    xtr.set_yticks([0, 1000])
    xtr.set_yticklabels(['0','1000'])
    ...
    xtr.spines['right'].set_visible(False)
    xtr.spines['top'].set_visible(False)
    xtr.set_yticks([0, 1000, 2000, 3000])
    xtr.set_yticklabels(['0','','','3000'])
    xtr.set_xticks([0, 1, 2, 3, 4])
    xtr.set_xticklabels(['0', '1', '2','3','4'])
    xtr.tick_params(axis="both",direction="in")
