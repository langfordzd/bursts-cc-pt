...
def plot_freq_check(lfp,lfp_bs,lfp_trials,lfp_ids,ecog,ecog_bs,ecog_trials,ecog_ids,restrict):
    ...
    for pos, r in zip(positions,restrict):
        ...
        ####################
    #%%    
def plot_chars_behav(ecog,marco,pablo,v):
    import matplotlib.pyplot as plt
    import matplotlib.gridspec as gridspec
    import seaborn as sns
    ...
    plt.show()
#%%    
def plot_behav(lfp,lfp_bs,lfp_trials,lfp_ids,ecog,ecog_bs,ecog_trials,ecog_ids,restrict):
    import matplotlib.pyplot as plt
    import matplotlib.gridspec as gridspec
    import seaborn as sns
    import pandas as pd 
    
    pt_lfp_b, pt_lfp_c      = behav(lfp,lfp_bs,lfp_trials,'pta', lfp_ids)
    cc_lfp_b, cc_lfp_c      = behav(lfp,lfp_bs,lfp_trials,'cc', lfp_ids)
    pt_ecog_b, pt_ecog_c    = behav(ecog,ecog_bs,ecog_trials,'pta',ecog_ids)
    cc_ecog_b, cc_ecog_c    = behav(ecog,ecog_bs,ecog_trials,'cc',ecog_ids)
    ...
    xtr = fig.add_subplot(gs[0:4, 2:7])
    palette = {"cc": qs[0], "pta": qs[1]}
    kws = {"s": 60, "facecolor": "none", "linewidth": 1.5}
    xtr = sns.scatterplot(
        data=behav_lfp, x="ratio_behav", y="ratio_burst", 
    ...
    xtr = fig.add_subplot(gs[6:7, 0:2])
    cecog = count_ecog[count_ecog['behav']=='repeat']
    xtr = sns.countplot(data = cecog, x='count',hue='which', palette=[qs[1],qs[0]])
    xtr.set_ylim([0, 7000])
    xtr.set_xlim([-0.5, 4.5])
    ...
    xtr = fig.add_subplot(gs[4:8, 2:7])
    xtr = sns.scatterplot(
        data=behav_ecog, x="ratio_behav", y="ratio_burst", 
        edgecolor=behav_ecog["which"].map(palette),
        **kws,
    ...
    for pos, r in zip(positions,restrict):
        ...
        xtr = fig.add_subplot(gs[pos[0]:pos[1], pos[2]:pos[3]])
        palette = {"cc": qs[0], "pta": qs[1]}
        kws = {"s": 60, "facecolor": "none", "linewidth": 1.5}
        xtr = sns.scatterplot(
            data=behav_lfp, x="ratio_behav", y="ratio_burst", 
        ...
        title = str(r[0])+'-'+str(r[1])+' Hz'
        xtr.set_title(title, x=0.2,y=0.8, pad=-14, rotation = 33)
        ...
        xtr = fig.add_subplot(gs[pos[4]:pos[5], pos[6]:pos[7]])        
        xtr = sns.scatterplot(
            data=behav_ecog, x="ratio_behav", y="ratio_burst", 
            edgecolor=behav_ecog["which"].map(palette),
            **kws,
